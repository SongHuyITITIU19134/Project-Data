...
d3.csv("Federal Firefighting Costs (Suppression Only).csv", rowConverter, function(err, data) {
    if (err) {
        console.log(err)
    } else {
        console.log(data)
...
function draw(data) {
    ...
    var yScale = d3.scaleLinear().domain([0, 3500000000]).range([h - pad, 0])
    ...
    bars.append("rect").attr("width", xScale.bandwidth() - 3)
        ...
        .attr("y", d => yScale(d.total) - pad)
        .attr("fill", d => "rgb(" + yScale(d.total) + ",128,128)")
        .on("mouseover", function(d) {
            var x = d3.event.pageX
            var y = d3.event.pageY

            d3.select("#value").text("Year: " + d.year)
            d3.select("#value2").text("Total costs: " + d.total + "$")
